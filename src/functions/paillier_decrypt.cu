...
namespace cuFIXNUM
{
    ...
    public:
        __device__ paillier_decrypt(fixnum p, fixnum q)
            : n(prod(p, q)), crt(p, q), decrypt_modp(p, n), decrypt_modq(q, n) {}
 ...
 */
    ...
    paillier_decrypt_mod<fixnum>::paillier_decrypt_mod(fixnum p, fixnum n)
        : p_sqr(square(p)), div_p(p), mod_p(p), mod_p2(p_sqr), pow(p_sqr, sub1(p))
    {
 ...
 */
    ...
    //*************** try to acclerate decrypt ****************
    template <typename fixnum>
    class paillier_decrypt_mod_debug
    {
    public:
        __device__ paillier_decrypt_mod_debug(fixnum p, fixnum n)
            : p_sqr(square(p)), div_p(p), mod_p(p), mod_p2(p_sqr), pow(p_sqr, sub1(p))
        {
            typedef typename fixnum::digit digit;
            digit cy;
    ...
    //merge paillier_decrypt_mod_debug
    template <typename fixnum>
    class paillier_decrypt_debug
    {
    public:
        __device__ paillier_decrypt_debug(fixnum p, fixnum q)
            : n(prod(p, q)), crt(p, q), 
            //init for paillier_decrypt_mod_debug
            p_sqr(square(p)), div_p(p), mod_p(p), mod_p2(p_sqr), pow(p_sqr, sub1(p))
        {
            //init for paillier_decrypt_mod
            typedef typename fixnum::digit digit;
            digit cy;
            fixnum t = n;
            cy = fixnum::incr_cy(t);
    ...
    private:
        ...
        //******** var and func in paillier_decrypt_mod_debug ********
        fixnum h;
        fixnum p_sqr;
        // Exact division by p
        divexact<fixnum> div_p;
